...
#pragma warning disable CA1812 //The class is instantiated by the Aspire framework
internal sealed class HealthChecksUILifecycleHook(DistributedApplicationExecutionContext executionContext) : IDistributedApplicationLifecycleHook
{
                            ...
                            && healthChecksEndpoint.Scheme is "http" && healthChecksEndpoint.TargetPort is int httpPort && httpPort != DefaultAspNetCoreContainerPort)
                        {
                            ...
                            context.EnvironmentVariables.Add(
                                ASPNETCORE_HTTP_PORTS,
                                    ReferenceExpression.Create($"{DefaultAspNetCoreContainerPort.ToString(CultureInfo.InvariantCulture)};{healthChecksEndpoint.Property(EndpointProperty.TargetPort)}"));
                        }
